        ...
        async Task SubmitRequest()
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    try
                    {
                        var document = new ReliefRequestForm(ReliefRequestVM);
                        ...
                        await _jSRuntime.InvokeVoidAsync("FileDownload", $"RS{ReliefRequestVM.DateRequested.ToString("MMddyyyy")}{ReliefRequestVM.RequestedById}", "application/pdf", fileContent);
                    }
                    catch (Exception ex)
                    {
                        await _jSRuntime.InvokeVoidAsync("Toast", "error", $"Something went wrong parsing your Request Form: {ex.Message}");
                    }
                    //}
